#Check boundaries of chunks to see if records (with same Name) spanned across 2 chunks, and if so, move them to 1 chunk
if (exists("ParFn_name_bndchk", mode="function"))
...
ParFn_name_match(data.frame(bdwlist[(((i-1)*6)+1):(i*6)], stringsAsFactors=FALSE))
}   

#Update revised Masterid, Description (returned by ParFn_name_match)

#Get updated/revised masterid, description (returned by ParFn_name_match)
New_Masterid <- NULL
Decription <- NULL
tmp <- NULL
